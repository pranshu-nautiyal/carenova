...
type Doc = { id: string; name: string; type: string; date: string; pages: number; source: string; status: string };

export default function DocumentsClient({ documents }: { documents: Doc[] }) {
  const [dragOver, setDragOver] = useState(false);
  const [selected, setSelected] = useState<Doc>(documents[0]);
    ...
    <div className="px-6 md:px-10 py-8 grid lg:grid-cols-[1.4fr_1fr] gap-6">
      <div>
        <div
          ...
          className={`card border-dashed !border-2 p-8 text-center transition-colors ${dragOver ? "bg-[color:var(--teal-soft)]" : ""}`}
        >
          ...
          <div className="text-xs text-[color:var(--ink-70)] mt-1">PDF, PNG, JPG · CareNova extracts entities to RxNorm, LOINC, ICD-10, SNOMED</div>
          ...
          <div className="card divide-y divide-[color:var(--line)]">
            {documents.map((d) => (
              <button
                ...
                className={`w-full text-left p-4 flex items-center gap-4 hover:bg-[color:var(--cream-2)] transition-colors ${selected.id === d.id ? "bg-[color:var(--cream-2)]" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[color:var(--ink-70)] mt-0.5">{d.type} · {d.pages}p · {d.date}</div>
                </div>
      ...
      <div className="card p-6 h-fit lg:sticky lg:top-24">
        ...
        <div className="mt-6 space-y-4">
          ...
          <ExtractRow label="Provider inferred" value="Auto-linked to attending" conf={0.94} />
          <ExtractRow label="Values found" value={`${selected.pages * 3}+ measurements`} conf={0.97} />
        </div>
        <div className="mt-6 pt-6 border-t border-[color:var(--line)]">
          ...
          <div className="flex flex-wrap gap-1.5">
            {["ICD-10 codes", "LOINC lab codes", "RxNorm meds", "SNOMED findings"].map((e) => (
        ...
        <div className="mt-6 pt-6 border-t border-[color:var(--line)]">
          ...
          <div className="text-xs text-[color:var(--ink-70)] leading-relaxed">
            This document is now linked to your active conditions, meds, and lab timeline.
          </div>
        </div>
      </div>
    </div>
