...
import { providers, patient } from "@/lib/mockData";
    ...
    <>
      ...
      <div className="px-6 md:px-10 py-8 space-y-8 max-w-6xl">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <SummaryCard
            title="Deductible"
            value={`$${patient.insurance.deductibleMet.toLocaleString()}`}
            of={`$${patient.insurance.deductible.toLocaleString()}`}
            pct={patient.insurance.deductibleMet / patient.insurance.deductible}
            note="After this, cost-share kicks in."
          />
          <SummaryCard
            title="Out-of-pocket max"
            value={`$${patient.insurance.oopMet.toLocaleString()}`}
            of={`$${patient.insurance.oopMax.toLocaleString()}`}
            pct={patient.insurance.oopMet / patient.insurance.oopMax}
            note="Above this, plan pays 100%."
          />
          <div className="card p-6">
            ...
            <div className="mt-2 text-xs text-[color:var(--ink-70)] leading-relaxed">
              Based on projected visits (3), prescriptions (12 refills), and one screening (eye exam).
            </div>
          </div>
        </div>
        ...
        <div className="card p-4 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <input
              type="text"
              placeholder="Search by specialty, name, or condition (e.g. 'diabetic eye exam')"
              className="flex-1 bg-transparent outline-none text-sm py-1"
            />
          </div>
          {["Within 10 mi", "In-network", "Telehealth OK"].map((f) => (
            <span key={f} className="chip chip-teal">{f}</span>
          ))}
        </div>

        {/* CareNova recommended */}
        <div>
          <div className="flex items-baseline justify-between mb-3">
            <div>
              <div className="text-sm font-medium">CareNova recommends</div>
              <div className="text-xs text-[color:var(--ink-70)]">Based on active needs in your graph: diabetic eye exam due, cough follow-up.</div>
            </div>
          </div>
        ...
        <div>
          <div className="text-sm font-medium mb-3">All in-network providers</div>
          <div className="space-y-3">
    ...
    <div className="card p-5 flex flex-col md:flex-row md:items-center gap-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex flex-wrap gap-3 text-xs text-[color:var(--ink-70)]">
          ...
          {p.inNetwork && <span className="text-[color:var(--leaf)]">· In-network</span>}
        </div>
      </div>
