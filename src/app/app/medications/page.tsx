...
export default async function MedicationsPage() {
  const { medications, interactions } = await getActivePatient();
  return (
    <>
      <Topbar
        title="Medications"
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`chip ${i.severity === "moderate" ? "chip-coral" : i.severity === "minor" ? "chip-amber" : "chip-teal"}`}>{i.severity}</span>
                      <span className="text-sm font-medium">{i.drugs.join(" ✕ ")}</span>
                    </div>
              ...
              <div key={m.id} className="card p-6">
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-1.5 h-1.5 bg-[color:var(--cream-2)] rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${m.adherence > 0.9 ? "bg-[color:var(--leaf)]" : m.adherence > 0.8 ? "bg-[color:var(--amber)]" : "bg-[color:var(--coral)]"}`} style={{ width: `${m.adherence * 100}%` }} />
                  </div>
                </div>
                {m.sideEffects.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-[color:var(--line)]">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {m.sideEffects.map((s) => (<span key={s} className="chip !text-[10px]">{s}</span>))}
                    </div>
                  </div>
                )}
                {m.nextRefill && (
                  <div className="mt-4 pt-4 border-t border-[color:var(--line)] flex items-center justify-between text-xs">
                    ...
                    <span className={m.refillsLeft && m.refillsLeft <= 1 ? "text-[color:var(--coral)]" : "text-[color:var(--ink-70)]"}>{m.refillsLeft} left</span>
                  </div>
                )}
              </div>
            ))}
